...
- The task within  `GAMLSS` is to find ;

    1)  how the x's effect the parameters of the distribution i.e $\boldsymbol{\theta}(X)$ and 
    
    2)  the appropriate distribution for the response i.e. $D(Y|\boldsymbol{\theta}(X))$


## DISTRIBUTION 

- The `range` of the response should determine the type of distribution 

- if we know the right variables we could determine a `adequate` distribution by looking over a set of appropriate distributions

- if `one` of the variable is very important i.e. time, age it can be used otherwise use a `linear` model for all parameters (which works fast) to pick up a distribution
...
mlinear <- gamlss2(rent~poly(area,3)+poly(yearc,3)+location+bath+kitchen+
              cheating|poly(area,3)+poly(yearc,3)+location+bath+kitchen+cheating, 
...
GAIC(mlinear, madditive, k=log(dim(rent99)[1]))
```
...
GAIC(mlinear, madditive, mregtree, k=log(dim(rent99)[1]))
```
...
gamlss2::GAIC(mlinear, madditive, mneural, mregtree, k=log(dim(rent99)[1]))
```
...
-   `la()` which fit the model using a modified smoothing technique

-   `lasso()`  which connects `gamlss2` and `glmnet`

    -  the `glmnet` provides elastic net estimation  with  `alpha=1` equal to lasso and `alpha=0` equal to ridge regression    

    -  the `glmnet` do not fit a single model but provides the path of all possible models

    -  in `gamlss2` the model is selected using `BIC`
...
da <- rent99[,-c(2,9)] 
 da |> data_factor() -> da
 da |> data_scale(, response="rent") -> da 
 da |> data_vars2data(,response="rent", nonlinear=TRUE, 
                      basis="poly", arg="3, raw=TRUE") -> da
 
# dafnet2  <- rent ~ lasso(~(area + area2 + area3 + yearc + yearc2 + yearc3 + 
#           bath1 + cheating0+ kitchen1 + location1+ location2)^2) |
#  lasso(~(area + area2 + area3 + yearc + yearc2 + yearc3 + 
#           bath1 + cheating0+ kitchen1 + location1+ location2)^2)|
#  lasso(~(area + area2 + area3 + yearc + yearc2 + yearc3 + 
#           bath1 + cheating0+ kitchen1 + location1+ location2)^2)
fnet1  <- rent ~ lasso(~area + area2 + area3 + yearc + yearc2 + yearc3 + 
          bath1 + cheating0+ kitchen1 + location1+ location2) |
lasso(~area + area2 + area3 + yearc + yearc2 + yearc3 + 
          bath1 + cheating0+ kitchen1 + location1+ location2) |
lasso(~area + area2 + area3 + yearc + yearc2 + yearc3 + 
          bath1 + cheating0+ kitchen1 + location1+ location2) |
lasso(~area + area2 + area3 + yearc + yearc2 + yearc3 + 
          bath1 + cheating0+ kitchen1 + location1+ location2)
bnet <- gamlss2(fnet1, data = da, family = BCTo)
gamlss2::GAIC(mlinear, madditive, mneural, mregtree, mcf, bnet, k=0)
...
## LASSO 2
```{r}
stmu<- specials(bnet, model = "mu")
stmu$coefficients

stsigma<- specials(bnet, model = "sigma")
stsigma$coefficients

stnu<- specials(bnet, model = "nu")
stnu$coefficients
```
