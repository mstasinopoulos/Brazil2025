...
```{R}
model_qqplot(m1, m4, m5)+
       theme_bw(base_size = 15)
```

::: aside
QQ-plot for linear `m1`, regression tree `m4` and gamma `m5` models  
:::
...
::: aside
Worm-plot for linear `m1`, regression tree `m4` and gamma `m5` models  
:::

## Example: diagnostics 3

```{R}
moment_bucket(m1, m4, m5)+
   theme_bw() + 
   theme(legend.position = "none")
```
::: aside
Bucket-plot for linear `m1`, regression tree `m4` and gamma `m5` models  
:::
...
-   Machine Learning methods are useful (especially for modelling interactions between variables) but they are not suitable if the interest do `not` lie in the mean of the distribution.
